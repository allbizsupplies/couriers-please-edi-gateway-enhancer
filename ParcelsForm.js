...
class ParcelsForm {
  constructor(consignmentForm) {
    ...
    this.totalWtField = this.consignmentForm['cd_deadweight']
  ...
  updateTotals() {
    ...
    this.totalWtField.value = totalWeight
  ...
  hide() {
    ...
    this.totalQtyField.value = 1
    this.totalWtField.value = ''
...
class ParcelsFormItem {
  ...
  build() {
    ...
    topRow.appendChild(
      this.buildPresetSelector(this.presets)
    )
    ...
    this.wtField = this.buildIntegerField()
    bottomRow.appendChild(this.wtField)

    bottomRow.appendChild(
      document.createTextNode(" kg (per item)")
    )
  ...
  applyPreset(id) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (readOnly)
        this.dimFields[i].style.color = 'grey'
      else
        this.dimFields[i].style.color = 'inherit'
    }
  }
  ...
  getVolume() {
    let volume = 1
    for (let i = 0; i < 3; i++) {
      const dim = parseInt(this.dimFields[i].value)
  ...
  getWeight() {
    const weight = parseInt(this.wtField.value)
